...
build_pkg() {
    ...
    grep -rqF "$sentinel" "$tree" 2>/dev/null \
        || die "sentinel '$sentinel' missing after patch — $src not patched"
...
main() {
    ...
    build_pkg kwin     "$kwin_patch" "buffer pixel space"
    build_pkg xwayland "$xwl_patch"  "falling back to"
